...
{
    public float speed = 2f;             // Define a velocidade da raquete
    ...
    void Update(){
            ...
            if(distance > 6f){
                Vector2 ir = new Vector2(0f,-3f);
                distance = Vector2.Distance(playerPos, ir);
                if(distance > 0.1f){
                    Vector3 dir = ir - playerPos;
                    dir.Normalize();
                ...
                float dis = Vector2.Distance(playerPos, ballPos);
                if(dis > 0.4f){
